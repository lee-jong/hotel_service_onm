...
class Create extends Component {
    ...
    switch (field) {
      ...
      case 'remarks':
        if (e.target.value.length > 500) {
          this.setState({
            remarksValidationError: '객실 메시지는 최대 500자 등록 가능합니다.'
          });
        } else {
  ...
  createManagementForm = async () => {
    ...
    if (confirm('계정을 생성하시겠습니까?') == true) {
      try {
        ...
        if (res.code === 200) {
          alert('계정이 생성되었습니다.');
          const href = `/management?admin_id=${hotelInfo.admin_id}&b2b_seq=${
            hotelInfo.b2b_seq
          }`;
          Router.push(href);
        }
  ...
  goToListPage = () => {
    ...
    if (confirm('작성 중인 내용이 취소됩니다.') == true) {
      const href = `/management?b2b_seq=${hotelInfo.b2b_seq}&admin_id=${
        hotelInfo.admin_id
      }`;
      Router.push(href);
    } else {
                  ...
                  <tr>
                    ...
                    <td>
                      <div className="textarea-wrap">
                        <textarea
                          placeholder="객실메시지는 최대 500자 등록 가능합니다."
                          className="email"
                          onChange={this.handleChangeInput}
                          name="remarks"
                        />
